...
export default function ReviewSubmit({ data }: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 max-w-2xl mx-auto text-center">
      <h2 className="text-4xl font-extrabold text-white mb-2">The 2026 NBA Champion</h2>
      ...
      <div className="bg-neutral-800/80 p-6 rounded-2xl text-left mb-8 space-y-4 shadow-xl border border-neutral-700">
        <h4 className="text-xl font-bold text-white border-b border-neutral-700 pb-2">Prediction Summary</h4>
        <div className="grid grid-cols-2 gap-4 text-sm text-neutral-300">
          <div><span className="text-neutral-500">Player:</span> {data.name}</div>
          <div><span className="text-neutral-500">Final Score:</span> {data.finalScore}</div>
          <div><span className="text-neutral-500">Finals MVP:</span> {data.finalsMVP}</div>
          <div><span className="text-neutral-500">Champion:</span> {champion.name}</div>
        </div>
      </div>
      ...
      {status === 'submitting' && (
        <div className="flex items-center justify-center py-5 text-orange-500 gap-3">
          ...
          <span className="font-bold text-xl">Locking in Prediction...</span>
        </div>
      )}
      ...
      {status === 'success' && (
        <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="flex flex-col items-center py-5 text-green-500">
          ...
          <span className="font-bold text-2xl text-white">Prediction Locked!</span>
          <p className="text-neutral-400 mt-2">Your choices have been saved and a confirmation email has been sent.</p>
        </motion.div>
      )}
